...
static int dmec_wdt_suspend(struct device *dev)
{
	struct dmec_wdt_data *wdat = dev_get_drvdata(dev);
	struct watchdog_device *wdd = &wdat->wdd;
...
static int dmec_wdt_resume(struct device *dev)
{
	struct dmec_wdt_data *wdat = dev_get_drvdata(dev);
	struct watchdog_device *wdd = &wdat->wdd;
...
static SIMPLE_DEV_PM_OPS(dmec_wdt_pm, dmec_wdt_suspend, dmec_wdt_resume);
#define DMEC_WDT_PM	(&dmec_wdt_pm)
#else
#define DMEC_WDT_PM	NULL
#endif
